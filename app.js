...
async function initializeMaps() {
  ...
  try {
    ...
    if (metadata?.vector_layers?.[0])
      sourceLayerName = metadata.vector_layers[0].id;
  } catch (err) {
    console.warn("Could not inspect PMTiles:", err.message);
  }
  ...
  const baseStyle = {
    ...
    sources: {
      ...
      parcels: {
        type: "vector",
        url: pmtilesUrl,
        attribution: "© Property Assessment Data",
      },
    },
    layers: [
      {
  ...
  try {
    ...
    residentialMap = new maplibregl.Map({
      ...mapConfig,
    ...
    await new Promise((resolve) => {
      ...
      const checkData = () => {
        attempts++;
        const features = residentialMap.querySourceFeatures("parcels", {
        ...
        if (features && features.length > 0) {
          ...
          residentialMap.triggerRepaint();
        } else {
          console.warn("Timeout waiting for tiles");
          resolve();
        }
      };
    ...
    if (
      ...
      err.message?.includes("parcels.pmtiles")
    )
      showPMTilesError(err.message);
  ...
  map.addLayer({
    ...
    paint: {
      "fill-color": colorExpr(
        "Zone",
        SYMBOLIZATION_OPTIONS.find((o) => o.value === "Zone"),
      ),
      "fill-opacity": [
        "case",
...
function attachMapHandlers(map, type, sourceLayer) {
  ...
  map.on("mousemove", "parcels-fill", (e) => {
    ...
    if (hoveredId !== null && hoveredId !== fid) {
      map.setFeatureState(
        { source: "parcels", sourceLayer, id: hoveredId },
        { hover: false },
      );
    }
    hoveredId = fid;
    ...
    hoverPopup
      .setLngLat(e.lngLat)
      .setHTML(
        `<div class="map-popup"><strong>${p["Property Address"] || "Unknown"}</strong><br><span>${p["Neighborhood"] || ""} · ${p["Zone"] || ""}</span><br><span class="popup-val">${fmt(val)}</span></div>`,
      )
      .addTo(map);
  });
...
function setSelectedParcelHighlight(map, sourceLayer, parcelId, type) {
  ...
  const features = map.querySourceFeatures("parcels", {
    sourceLayer,
    filter: ["==", ["get", "Parcel ID"], parcelId],
  });
  if (!features.length) return;
  map.addSource("parcel-selected", {
    type: "geojson",
    data: { type: "Feature", geometry: features[0].geometry, properties: {} },
  });
  map.addLayer({
    id: "parcel-selected",
    type: "line",
...
async function collectParcelData(sourceLayer = "parcels") {
  ...
  if (!features || features.length === 0) {
    showError("No features found in PMTiles file.");
    return;
  }
  ...
  features.forEach((f) => {
    ...
    if (pt === "Residential" || pt.includes("Residential"))
      buckets.residential.push(f.properties);
    else if (pt === "Condo" || pt === "Condominium")
      buckets.condo.push(f.properties);
    else if (pt === "Commercial") buckets.commercial.push(f.properties);
    else if (pt === "Vacant" || pt === "Vacant Land" || pt.includes("Vacant"))
      buckets.vacant.push(f.properties);
  });
  ...
  Object.entries(buckets).forEach(([type, props]) => {
    ...
    const vals2022 = parcels
      .map((p) => p.assessed2022)
      .filter((v) => v > 0)
      .sort((a, b) => a - b);
    const mid = Math.floor(vals2022.length / 2);
    const median2022 = vals2022.length
      ? vals2022.length % 2 === 0
        ? (vals2022[mid - 1] + vals2022[mid]) / 2
        : vals2022[mid]
      : 0;
    statsData[type] = {
...
function showParcelDetail(props, type) {
  ...
  const rows = [
    ...
    [
      "Living Area",
      props["Living Area"]
        ? Math.round(parseFloat(props["Living Area"])).toLocaleString() + " sf"
        : null,
    ],
    [
      "Acres",
      props["Land Acres"]
        ? parseFloat(props["Land Acres"]).toFixed(2) + " ac"
        : null,
    ],
    ...
    pct !== null
      ? [
          "Change",
          `${changeFmt(a25 - a20)} (${up ? "▲" : "▼"}${Math.abs(pct)}%)`,
        ]
      : null,
  ...
  el.innerHTML = `<div class="prop-detail">
    ...
    ${rows
      .map(([l, v]) => {
        ...
        const lbl = tip
          ? `<span class="prop-row-label" title="${tip}">${l} <span class="field-tip">ⓘ</span></span>`
          : `<span class="prop-row-label">${l}</span>`;
...
function highlightParcelOnMap(parcelId, type) {
  ...
  if (!features.length) return;
  map.fitBounds(getBBox(features[0].geometry), {
    padding: 100,
    duration: 1000,
  });
}
...
function getBBox(geometry) {
  if (geometry.type === "Polygon") {
    ...
    const all = geometry.coordinates.flat(2),
      lngs = all.filter((_, i) => i % 2 === 0),
      lats = all.filter((_, i) => i % 2 === 1);
    return [
      [Math.min(...lngs), Math.min(...lats)],
      [Math.max(...lngs), Math.max(...lats)],
    ];
...
function switchTab(tab) {
  ...
  lazyInitMap(tab).then(() => {
    ...
    if (dataCollected[tab]) {
      ...
      if (scatterPending[tab]) {
        updateScatter(tab);
        scatterPending[tab] = false;
      }
    }
    // Restore selected parcel highlight
    if (selectedParcelId[tab] && maps[tab]) {
      setSelectedParcelHighlight(
        maps[tab],
        _sourceLayerName,
        selectedParcelId[tab],
        tab,
      );
    }
    // Update filter chip
...
function updateResBedroomBeeswarm(parcels) {
  ...
  const _beeColorFn_resBed = buildBeeswarmColorFn(
    _beeOpt_resBed,
    "residential",
  );
  watchAndRender(container, () =>
...
window.updateRightBarChart = function (type, field) {
  ...
  watchAndRender(container, () =>
    renderBarInto(container, data, COLORS[type], type, field),
  );
};
...
window.toggleSidebar = function (type, side) {
  ...
  if (side === "left" && leftPanel)
    leftPanel.style.marginLeft = state.left
      ? `-${leftPanel.offsetWidth}px`
      : "0";
  if (side === "right" && rightPanel)
    rightPanel.style.marginRight = state.right
      ? `-${rightPanel.offsetWidth}px`
      : "0";
  const btn =
    side === "left"
      ? dashboard.querySelector(".dash-left .sidebar-toggle")
      : dashboard.querySelector(".dash-right .sidebar-toggle");
  if (btn) {
    const label = state[side]
      ? side === "left"
        ? "›"
        : "‹"
      : side === "left"
    ...
    btn.setAttribute(
      "aria-label",
      state[side] ? `Expand ${side} panel` : `Collapse ${side} panel`,
    );
  }
  const maps = {
    residential: residentialMap,
...
window.toggleParcelFilter = function (type) {
  ...
  if (btn) {
    btn.textContent = showAllParcels[type]
      ? "Showing all types"
      : "Filter to " + typeLabel + " only";
...
(function initResizableSidebars() {
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    const sidebar = currentHandle.parentElement,
      dashboard = sidebar.parentElement;
...
function updateScatter(type) {
  ...
  if (countEl)
    countEl.textContent = ` (${sample.length.toLocaleString()} sampled)`;
  watchAndRender(container, () =>
...
function renderBarInto(container, data, color, type, field, activeVal) {
  ...
  const margin = {
    top: 6,
    right: Math.round(fs * 5.5),
    bottom: Math.round(fs * 5.5),
    left: Math.round(Math.min(width * 0.4, fs * 12)),
  };
  ...
  const xAxis = d3
    .axisBottom(xScale)
    .ticks(Math.max(3, Math.floor(iW / 50)))
    .tickFormat(fmtAxis);
  ...
  yAxisG
    ...
    .each(function (d) {
      ...
      if (d && d.length > maxChars)
        d3.select(this).text(d.slice(0, maxChars - 1) + "…");
    });
  const bars = g
    .selectAll(".bar")
    .data(data)
    ...
    .attr("fill-opacity", (d) =>
      activeVal && d.label !== activeVal ? 0.35 : 1,
    )
    .attr("rx", 2);
  ...
  g.selectAll(".bar-hover")
    ...
    .on("mouseover", function (event, d) {
      tooltip
        ...
        .classed("show", true)
        .html(
          `<strong>${d.label}</strong><br>Parcels: ${d.count.toLocaleString()}<br>Mean 2022 Assessment: ${fmtVal(d.mean)}`,
        );
...
function applyMapFilter(type) {
  ...
  const tileField = {
    ...
    style: "Style Description",
    zone: "Zone",
    stateUse: "State Use Description",
    frame: "Frame Type",
  };
...
function getTypeFilterExpr(type) {
  ...
  if (type === "vacant")
    return [
      "any",
      ["==", ["get", "Property Type"], "Vacant"],
      ["==", ["get", "Property Type"], "Vacant Land"],
    ];
  if (type === "condo")
    return [
      "any",
      ["==", ["get", "Property Type"], "Condo"],
      ["==", ["get", "Property Type"], "Condominium"],
    ];
  return [
    "==",
    ["get", "Property Type"],
